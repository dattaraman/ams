<?php
    ...
    $output = '';
    if(isset($_POST['ser'])){
        $usr = $getdata->getupapproved($_POST['ser']);
    }else{
    ...
    foreach ($usr as $row){
        $aimsid_show = '';
        if($row['aimsid']!='0'){
            $aimsid_show = ' <li class="nav-item listitem">
            <span class="nav-link text-center text-secondary">
            <span class="fas fa-id-badge"></span> '.$row["aimsid"].'
        ...
        <div class="widget-user-header bg-dark">
              ...
              <div class="card-footer p-0">
                ...
                <li class="nav-item listitem">
                    ...
                    <li class="nav-item listitem">
                    <span class="nav-link">
                    <span class="fas fa-thumbs-up" data-toggle="tooltip" title="Group-Admin Approved on"></span> <span class="text-uppercase">'.date('d-m-Y H:m:s',strtotime($row['groupadmin_approvedon'])).'</span>
                ...
                <span class="fas fa-house-user"> '.$row["pc_location"].' </span>
                </span>
            </li>
                ...
                <li class="nav-item listitem">
                <span class="nav-link">
                <span class="fas fa-desktop"></span> '.$mk.'
                <span class="fas fa-dice-five mr-1 float-right"> '.$md.'</span>
                </span>
            </li>
                    ...
                    <li class="nav-item listitem">
                    <span class="nav-link">
                    <span class="fas fa-hdd mr-1"></span>'.$row["pc_hdd"].' GB
                    <span class="fab fa-sellcast m-1 float-right"> '.$row["pc_ssd"].' GB</span>
                    </span>
                </li>
   ...
   if(isset($_POST['operation']) && $_POST['operation'] == 'alllistdevice'){
    ...
    $output = '';
    if(isset($_POST['ser'])){
        $usr = $getdata->getupapproveddevice($_POST['ser']);
    }else{
    ...
    foreach ($usr as $row){
        $aimsid_device_show = '';
        if($row['aimsid']!='0'){
            $aimsid_device_show = ' <li class="nav-item listitem">
            <span class="nav-link text-center text-secondary">
            <span class="fas fa-id-badge"></span> '.$row["aimsid"].'
        ...
        if($row['device_source']=='Centralize'){
            ...
            $mk = $getdata->getdevicemakebymakemodel($row['device_make']);
            $md = $row["device_model"];
            $dv = $row['device'];
        }
        ...
        <div class="widget-user-image">
            ...
            <li class="nav-item listitem">
            <span class="nav-link">
            <span class="fas fa-hdd mr-1"></span>'.$mk.'
            <span class="fab fa-sellcast m-1 float-right"> '.$md .'</span>
            </span>
        </li>
   ...
   if(isset($_POST['operation']) && $_POST['operation'] == 'checkaimsid'){

    $res = $getdata->checkaimsid($_POST['id']);
   ...
   if(isset($_POST['operation']) && $_POST['operation'] == 'checkaimsid_device'){
   ...
   if(isset($_POST['operation']) && $_POST['operation'] == 'getaimsidmax'){

    $res = $getdata->getmaxaimsid();
    if($res){
        echo $res;
    }
    else{
   ...
   if(isset($_POST['operation']) && $_POST['operation'] == 'getaimsidmax_device'){

    $res = $getdata->getmaxaimsid_device();
    if($res){
        echo $res;
    }
    else{
   ...
   if(isset($_POST['operation']) && $_POST['operation'] == 'admin_mark_approve'){

    $res = $getdata->adminapproval($_POST['id'],$_POST['aimsid']);
    
    if ($res){

        $getdata->log_activity('[SYS-ADMIN] PC Approved : ID-'.$_POST['id'].', AIMS ID : '.$_POST['aimsid']);
        echo 1;
    }
    else{
        echo 0;
   ...
   if(isset($_POST['operation']) && $_POST['operation'] == 'admin_mark_approvedevice'){

    $res = $getdata->adminapprovaldevice($_POST['id'],$_POST['aimsid']);
    
    if ($res){

        $getdata->log_activity('[SYS-ADMIN] PRINTER/SCANNER Approved : ID-'.$_POST['id'].' AIMS ID '.$_POST['aimsid']);
        echo 1;
    }
    else{
        echo 0;
